...
def launch_store():
	...
	time.sleep(6)
	if device(text="GAMES").exists :
		device(text="GAMES").click()
		print "Success ; GAMES Tab Launched"
		time.sleep(1)
...
def install(n):
	...
	while True :
		if device(textStartsWith=num).exists :
			...
			print "Success ; Game%s Details Launched" % n
			time.sleep(1)
			break
		else :
#			device(textStartsWith=lastnum).swipe.up(steps=400)
	...
	while True :
		if device(text="OPEN").exists :
			print "Already Installaed Skipping Installation of Game %s" % n
			time.sleep(2)	
			break
		if device(text="INSTALL").exists :
			device(text="INSTALL").click()
			install_needed=1
			print "Success ; Game%s Install Clicked; Waiting for permission accept.." % n
			time.sleep(2)	
			break
		if device(description='Cancel this download' , className='android.widget.ImageView').exists :
			print "Already installation in progress Skipping Installation of Game %s" % n
			time.sleep(2)	
			break
	...
	while True :
		if device(text="OPEN").exists :
			...
			break
	'''
	device.screenshot("Game"+str(n)+".jpg")
	print 'Going back to game list'
	device.press.back()
...
def main():
	...
	launch_store()
	if sys.argv==2 :
		print 'Installing Top %s Games ' % sys.argv[2]
		n=int(sys.argv[1])
	else :
		print 'Default : Installing Top 50 Games '
		n=50
	for i in xrange(1,n+1):
		print "Install Game %s  " % i
		install(i)
